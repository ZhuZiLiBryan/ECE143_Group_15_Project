...
def plot_transactions_by_hour(df, save_path='sales_by_hour.png'):
...
if __name__ == "__main__":
    ...
    if df is not None:
        # 2. Visualization (Sales by Hour)
        plot_transactions_by_hour(df)
        
        # 3. RFM Feature Engineering
        rfm_df, X_scaled, features = create_rfm_features(df)
        
        # 4. Elbow Method (Visualize to choose k)
        plot_elbow_method(X_scaled)
        
        # 5. Final Clustering (Assuming k=3 based on Elbow plot)
